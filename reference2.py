...
def build_pairs(cluster):
    ...
    for pa in cluster:
        if is_question_pair(pa):
            pair['question_substring'] =  getIndexs(pa['text'])
            continue
        else:
            answer_substrings.append(filteringMentions(pair['question_substring'],pa['text']))

    answer_substrings = list(set([x for x in answer_substrings if x is not None]))
    if(len(answer_substrings) == 0):
        return {}

    pair['answer_substrings'] = list(itertools.chain(*[getIndexs(item) for item in answer_substrings]))
    return pair
